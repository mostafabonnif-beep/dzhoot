...
- **التحقق الحي للتشغيل**: جهاز المستخدم سجّل `startup_success` متعددة (أحدها 0.9s) وشاهد مقاطع حتى #94؛ قناة TRI BEIN SPORTS GOLD اختُبرت من الخادم: توكن/مانيفست/مقطع كلها 200 (7.1MB MPEG-TS). **زمن بدء 10-15s لقنوات Upstream = زمن استجابة المزود عبر الوكيل، وليس خللًا.**
...
- [ ] الـ 40 بثًا "متوقفًا" في فحص liveness: **متوقع** لأن المزود (Primary Upstream) يحجب IP الداتاسنتر — القياس الخادمي مستحيل، تُقاس بفشل التشغيل الفعلي (موثق في FINAL_REPORT الصباحي PR #27).
...
## 8. ميزة جديدة (جلسة 2026-08-24): المحلات والدفعات (Resellers + Code Batches)

نُفّذت بالكامل ومنشورة (commits `1f8e369` + `8e55a67`، نشر v1.0.1-20260824T025954Z + إعادة بناء صورة API):

- **Reseller (محل)**: نموذج + CRUD (`/api/v1/admin/resellers`) — الاسم، المدينة، الهاتف، ملاحظات، حالة؛ مع إحصائيات أكواد كل محل (إجمالي/مفعّل/متبقي).
- **CodeBatch (دفعة)**: كل إرسالية لمحل = دفعة برقم تسلسلي (دفعة 1، 2…) + الخطة (المدة) + الكمية + **تاريخ الاستلام**. `POST /api/v1/admin/code-batches` يولّد الدفعة وأكوادها دفعة واحدة ويعيد الأكواد كنص؛ `GET /:id/export` يصدّر **ملفًا جاهزًا للطباعة/التسليم** (ترويسة: المحل/المدينة/التاريخ/المدة + قائمة الأكواد).
- الأكواد الآن مرتبطة بـ `resellerId` + `batchId`؛ قائمة الأكواد تعرض المحل والدفعة مع فلاتر.
- **اللوحة**: صفحتا `/admin/resellers` و `/admin/code-batches` (عربي RTL) + تحديث صفحة الأكواد + الشريط الجانبي + الترجمات (ar/en/fr).
- **الخطط القياسية** (مُضافة لقاعدة الإنتاج): تجربة 7d / شهري 30d / 6 أشهر 180d / سنوي 365d — الأسعار 0 (تُحدَّث من صفحة الباقات).
- ملاحظة تقنية: النماذج الجديدة يجب أن تصدّر بـ `module.exports = Model` + `export default` (نمط Plan.ts) وإلا يفشل `require()` في الحاويات.

**تحقق حي**: محل "بن عكنون / الجزائر العاصمة" + دفعة #1 (شهري، 3 أكواد) + تصدير الملف — كلها تعمل.
