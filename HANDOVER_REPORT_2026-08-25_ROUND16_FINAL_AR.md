...
| الجبهة | النتيجة |
|---|---|
| **المستودع** | ✅ `main` = `0c68ddd` — نظيف ومتطابق مع الخادم (10/10 ملفات sha256) |
...
| **البث المباشر** | ✅ مفعّل — الفيديو يخرج من NEO مباشرة (خادمك حر) |
| **التراجع التلقائي** | ✅ مباشر→بروكسي (خادم + تطبيق) — منشور ومُختبَر حيًا |
| **APK رسمي موقّع** | ✅ `dzhoof-release-1.0.10` (official/release) — يُوزَّع عبر التحديث الداخلي |
| **إصلاح التحديث** | ✅ «التواقيع غير متطابقة» أُصلح — `latest` = v1.0.10 الرسمي (كان v1.0.5 الـ staging) |
...
5. **البث المباشر مفعّل**: `ALLOW_DIRECT_PLAYBACK=true` (في .env **و** بلوك
   ...
   على مصدر Business Cloud NEO. الضغط كله على NEO (خلف Cloudflare)؛ الخادم
...
6. **التراجع التلقائي مباشر→بروكسي**: tv.js يرجع `proxyPlaybackUrl` (توكن بروكسي
   ...
   مباشر 302→NEO، بروكسي 200 قائمة عبر الخادم.
...
- الإنتاج متطابق مع main **0c68ddd** (10 ملفات sha256 — القائمة في القسم 6).
- التحديث الداخلي حي: AppVersion النشط = **1.0.10** (downloads/dzhoof-v1.0.10.apk،
  sha256 b0eefe47…، 77,406,384 بايت) + `releases/latest` = v1.0.10 الرسمي.
...
- [ ] **رقم Max Connections من لوحة NEO** — السقف الحقيقي للبث المباشر
...
- [ ] تنبيه أوتوماتيكي إذا NEO وقع (webhook — ينتظر عنوانًا من المستخدم).
...
2. **روابط NEO**: المدخل المستقر `cf.business-cloud-neo.ru/live/<t>/<t>/<id>.m3u8`
...
## 9. إصلاح التحديث الداخلي (خطأ «التواقيع غير متطابقة») — 2026-08-25 مساءً

### السبب الجذري
- Release GitHub **v1.0.5** (منشور 2026-08-19) يحمل APK اسمه
  `DZHOOF-TV-staging-v1.0.5-production.apk` (94.7MB) — **موقّع debug**.
- التطبيق المثبّت عند الزبائن **موقّع release** → أندرويد يرفض التحديث
  («التواقيع غير متطابقة»). كانت `releases/latest` ترجع هذا الملف (احتياط
  التطبيق عندما لا يرجع سجل الخادم تحديثًا).

### الإصلاح (نُفّذ ومُتحقق)
1. **versionCode 10009 → 10010** + الافتراضي 1.0.10 (commit `242774f`) —
   حتى أصحاب v1.0.9 (10009) يرون التحديث.
2. **Release رسمي 1.0.10** بُني عبر release-candidate.yml (نفس مفتاح التوقيع
   = المثبّت الرسمي) → `dzhoof-release-1.0.10` artifact.
3. **Release GitHub v1.0.10** أُنشئ والـ APK الرسمي رُفع كـ asset
   (`DZHOOF-TV-v1.0.10-production.apk`، 77,406,384 بايت) → `latest` = v1.0.10.
4. **سجل الخادم AppVersion**: سجل جديد active (versionCode 10010، versionName
   1.0.10) + الملف في `/opt/dzhoot/server/downloads/dzhoof-v1.0.10.apk` + sha256
   (b0eefe47…) — السجلات القديمة (1.0.7/1.0.8) عُطّلت.
5. **تحقق**: `/api/v1/app/version?currentVersion=10004` → updateAvailable:true،
   downloadUrl → `https://iptv.ld-11.net/downloads/dzhoof-v1.0.10.apk`
   (HTTP 200 بالحجم الكامل). GitHub `releases/latest` = v1.0.10.

### الإثبات
- 1.0.10 وكل البناءات الرسمية السابقة (1.0.2/1.0.9...) من **نفس الـ workflow
  ونفس أسرار التوقيع** (ANDROID_KEYSTORE_BASE64 + passwords) → نفس الشهادة →
  التحديث يثبت فوق أي نسخة رسمية مثبّتة.
- ⚠️ جهاز مُثبَّت عليه نسخة **staging/debug** سابقًا (مثل v1.0.5 الـ staging):
  يحتاج **إلغاء التثبيت ثم تثبيت الـ APK الرسمي** (لن يثبت فوق debug).

### ملاحظات
- Release v1.0.5 القديم (الـ staging) ما زال موجودًا على GitHub لكنه لم يعد
  `latest` — يمكن حذفه لاحقًا (اختياري).
- توزيع التحديث: التطبيق يسأل الخادم أولًا (سجل AppVersion) ثم GitHub —
  كلاهما الآن يقدّم 1.0.10 الرسمي.

## 8. الحالة النهائية

- GitHub `main` = `0c68ddd` = الخادم (10/10 sha256) — كل التغييرات مدفوعة.
- الإنتاج `v1.0.1-20260825T074000Z` سليم: بث مباشر + تراجع تلقائي + إيراد صحيح.
- **التحديث الداخلي يعمل**: 1.0.10 الرسمي يُقدَّم من الخادم + GitHub — الخطأ
  «التواقيع غير متطابقة» أُصلح (كان سببًا في release v1.0.5 الـ staging).
- APK الرسمي `1.0.10` جاهز للتوزيع اليدوي (الاحتياط يعمل مع 1.0.2+).
- أوراق الاعتماد (المفتاح، PAT، روابط اللوحات) في القسم 2.
- الخطوة الأولى للي بعدي: تذكير المستخدم بتغيير كلمتي المرور (admin + root)
  وطلب رقم Max Connections من لوحة NEO.
