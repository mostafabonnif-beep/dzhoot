...
## 18. Customer app polish update — 2026-08-15

تم تحسين واجهة العميل في Android بتعريب عناوين Home مثل «شاهدت مؤخرًا» و«مقترح لك»، وتعريب أزرار المشاهدة وعرض الكل ومعلومات البرنامج الحالي والقادم ورسائل إعادة الاتصال وفشل البث ولوحة بحث Android TV. كما أُعيد تصميم حالة عدم توفر المحتوى داخل بطاقة Material3 متجاوبة، مع إزالة أي رابط إدارة أو تفاصيل مصدر من الحالة.

تعذر البناء المحلي لـAndroid لأن البيئة الحالية لا تحتوي Android SDK؛ لا يمثل ذلك فشلًا في الكود، وسيبقى تحقق Android معتمدًا على CI عند توفر Actions. تم تشغيل فحوصات Backend وFrontend وCompose بنجاح قبل هذه الدفعة.


## 19. Subscription UX update — 2026-08-15

تمت إعادة صقل بوابة التفعيل ببطاقة متجاوبة، رمز بصري واضح، تنسيق تلقائي للكود، وزر تفعيل مناسب للهاتف وAndroid TV. كما أصبحت رسائل الاشتراك عربية وموجهة للعميل؛ أخطاء HTTP 400/401/403 ومشكلات الاتصال لا تُعرض كنصوص تقنية، بل تتحول إلى إرشادات مفهومة مثل صلاحية الكود أو انتهاء الاشتراك أو فشل الاتصال.

لم يتم تضمين أي عنوان خادم أو رابط مصدر أو تفاصيل Backend في الرسائل الجديدة.

## 20. Local customer-flow smoke — 2026-08-15

تم تنفيذ اختبار تكاملي على الخادم التجريبي: إنشاء كود اختبار جديد من لوحة الإدارة، تفعيله من مسار العميل، إنشاء جلسة عميل وجهاز Samsung S22 تجريبي، ثم قراءة الاشتراك من `/api/v1/me/subscription`. النتيجة: **نجاح 200**، والاشتراك ACTIVE بمدة 7 أيام وحد جهازين.

تم اختبار `/api/v1/streams/authorize` بجلسة العميل على معرّف محتوى غير موجود؛ أعاد الخادم `CONTENT_NOT_FOUND` دون إعادة أي `channelUrl` أو `streamUrl` أو `m3u8` أو بيانات Xtream. هذا يؤكد عدم تسريب المصدر في استجابة العميل. يبقى اختبار التشغيل الإيجابي من جهاز حقيقي معلقًا حتى إضافة قناة أو فيلم من مصدر قانوني ومرخّص، وهو مانع الإطلاق التجاري الأساسي.

## 21. Full technical audit — 2026-08-15

تم تشغيل Backend وFrontend على الخادم المؤقت والتحقق من `/health` وواجهة `/login` وCSS العام. نجحت فحوصات Backend بنتيجة 156/156 بعد إضافة اختبارات نطاق المحتوى، ونجحت Frontend tests وbuild وSmoke E2E وproduction Compose validation.

اكتشف الفحص مسارات كانت تعرض Home وCatalog وبحث القنوات دون تطبيق نطاق الخطة بصورة متسقة. تم إصلاحها وإعادة تشغيل Backend، ثم اختبار خطة QA ذات نطاق فارغ؛ أصبحت القوائم والتصنيفات والبحث وHome فارغة للعميل، دون أي حقل تقني أو رابط مصدر. تم توثيق التفاصيل في `docs/TECHNICAL_AUDIT_2026-08-15.md`.

أبلغ `npm audit --audit-level=high --omit=dev` عن 32 ثغرة في شجرة الاعتماديات، منها 12 عالية وواحدة حرجة. لم يُنفذ `npm audit fix --force` لأن بعض الإصلاحات المقترحة ترقيات كاسرة، ويجب التعامل معها في Sprint مستقل مع اختبارات وترقيات تدريجية.

## 22. Stable domain and VPS portability — 2026-08-15

النطاق الثابت يبقى خارج VPS لدى شركة التسجيل وDNS؛ لذلك يمكن نقل الخدمة بين Oracle وAWS وأي مزود آخر بتغيير سجل DNS فقط بعد نقل البيانات والأسرار واختبار الخادم الجديد. لا يحتاج APK إلى إعادة بناء عند تغير IP فقط، ما دام `PUBLIC_BASE_URL` وعقد API وFirebase ثابتة. أضيف دليل `docs/STABLE_DOMAIN_VPS_RUNBOOK_AR.md` للتنفيذ.
