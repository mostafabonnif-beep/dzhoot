...
هذا الدليل يشرح كيف تستعيد نفس إعدادات التجربة (قنوات Primary Upstream + iptv-org) على بيئة جديدة — ساندبوكس جديد، حساب MoClaw آخر، أو الـ VPS — **دون إعادة العمل يدويًا**.
...
3. أضف بيانات مصدر Upstream إلى `.env`:
   ```bash
...
> ملاحظة: استيراد كتالوج Upstream الكامل (16,609 قناة + أفلام + مسلسلات) يستغرق وقتًا. مرحلة حلقات المسلسلات بطيئة (16,710 مسلسل بتوازي 3) — جعلها lazy يُنصح بها كتحسين قادم.
...
2. استعد من الأرشيف الذي استلمته من الجلسة:
   ...
   ```
3. **مهم**: بيانات اعتماد المصادر مخزّنة مشفرة بمفتاح `XTREAM_SECRET_KEY` — استخدم نفس المفتاح من ملف الأسرار المرفق مع الأرشيف، وإلا فشل فك التشفير. أعد إدخال بيانات اعتماد Upstream يدويًا من لوحة الإدارة إن لزم.
