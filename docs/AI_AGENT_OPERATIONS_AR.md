...
3. تحقّق بعد البناء:
   ...
   - بعد أي تغيير في أصول إصدار، أبطِل كاش الـGitHub عبر واجهة الإدارة بدل مسح redis يدويًا:
     `POST https://iptv.ld-11.net/api/v1/admin/app-versions/cache/invalidate` (جلسة Admin، ويُسجَّل في سجل التدقيق). `500` تعني أن الكاش **لم** يُمسح.
