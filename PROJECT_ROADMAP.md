...
2. CI إلزامي أخضر قبل الدمج: backend (typecheck + lint + 152 اختبار + smoke E2E)، android (unit tests + assembleDebug + APK)، frontend (build + tests).
3. الفروع المندمجة تُحذف فوراً؛ أي فرع يبقى أكثر من أسبوع بلا PR يُسأل عنه.
4. تُوحَّد `develop → main` عند كل إصدار (`release/*`) وبعد كل حزمة عمل مكتملة.
