...
The V4 runner selects the first stream URL from a local M3U snapshot and tests it with curl, FFprobe, and headless VLC on the same execution host. Headers, raw logs, and FFprobe output remain in a mode-0700 private directory; the JSON summary redacts provider credentials and signed redirect parameters. VLC messages such as PulseAudio initialization errors are not treated as stream success, and media success requires actual detected media rather than HTTP 200 alone.
