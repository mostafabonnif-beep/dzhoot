...
## ⚡ تحديث — الجولة 9 (نفس اليوم): إكمال فكرة الموزع والأكواد

مطلوب المستخدم: "الموزع ينشئ الكود الخاص به حسب الرصيد الذي يشتريه، وعند التفعيل تبدأ الأيام تُحسب". الحلقة الأساسية كانت تعمل منذ جولة 7؛ الجولة 9 أكمَلتها:

1. **بادئة أكواد فريدة لكل محل** (`prefix` 3-6 أحرف، يضبطها الأدمن) — كل كود يحمل بادئة محله (`ALG1-XXXX-XXXX`) فتعرف مصدره.
2. **محاسبة المشتريات**: حركات المنح تسجّل `unitPrice` + `amount` (كمية × سعر الجملة)؛ الموزع يرى "حسابي": المُشترى (كمية/قيمة دج)، المُستهلك، الصافي.
3. **نوافذ الاشتراك في البوابة**: الكود المُفعَّل يعرض بداية الاشتراك ونهايته (من `ActivationRedemption → Subscription`) — الموزع يرى الأيام تُحسب فعليًا.
4. **تنبيه انتهاء قريب**: عدد الأكواد غير المستخدمة التي تنتهي خلال 7 أيام.

النشر `v1.0.1-20260824T153300Z` | GitHub `72347ce` | اختبارات 250/250 | تحقق حي كامل (بادئة TST9 + قيمة شراء 8000 + تفعيل كود → اشتراك 7 أيام يظهر للموزع ✓).

**درس مهم (خطأ فادح تجنّبناه):** `mongosh` داخل حاوية mongo يتصل بقاعدة `test` الافتراضية وليس قاعدة الإنتاج (`dzhoof-iptv`)! أي أمر mongosh للتنظيف يجب أن يحدد القاعدة: `db.getSiblingDB('dzhoof-iptv')` أو `mongosh dzhoof-iptv`. (في جولات 7-8 استخدمنا mongosh للتنظيف على القاعدة الخطأ — بلا ضرر لأنها كانت فارغة — ثم نظّفنا يدويًا القاعدة الصحيحة في جولة 9.)
