...
function LoginContent() {
  ...
  // Load capability flags so disabled features are hidden, not broken.
  useEffect(() => {
    api
      .get('/config/defaults')
      .then((res) => {
  ...
  return (
    <div className="w-full max-w-md rounded-[2rem] border border-border/70 bg-card/85 p-6 shadow-2xl shadow-slate-900/10 backdrop-blur-xl ring-1 ring-black/5 dark:ring-white/5 sm:p-9">
      ...
      {(capabilities.googleOAuthEnabled || capabilities.githubOAuthEnabled) && (
        <div className="mt-6 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {capabilities.googleOAuthEnabled && (
            <a
            ...
            {capabilities.githubOAuthEnabled && (
            <a
