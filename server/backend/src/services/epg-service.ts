...
const EPG_FETCH_CONCURRENCY = Math.max(1, parseInt(process.env.EPG_FETCH_CONCURRENCY || '1', 10) || 1);
...
// Decompressed XML cap per guide (was a fixed 200MB). One giant guide is the
// single biggest heap consumer; lowering the cap bounds worst-case memory.
const EPG_MAX_DECOMPRESSED_MB = Math.max(
  20,
  parseInt(process.env.EPG_MAX_DECOMPRESSED_MB || '100', 10) || 100,
);
...
// Heap guard: before starting each source, if the process heap is above this
// threshold the source is skipped and recorded as an error instead of letting
// the process crash with a fatal OOM (which killed the whole scheduler).
  ...
  private async doRefreshEpg(): Promise<void> {
    ...
    try {
      ...
      const batchStats = await runBoundedBatch(
        ...
        async (source) => {
          if (heapUsedMb() > EPG_HEAP_GUARD_MB) {
            throw new Error(`Skipped: heap ${heapUsedMb()}MB exceeds EPG_HEAP_GUARD_MB (${EPG_HEAP_GUARD_MB}MB)`);
          }
          ...
          try {
            ...
            console.log(
              `[epg-service] Source ${source.source}: heap ${beforeHeap}MB -> ${heapUsedMb()}MB (${Math.round((Date.now() - startTime) / 1000)}s elapsed)`,
            );
          }
        },
        {
