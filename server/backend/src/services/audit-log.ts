...
export function redactSensitiveText(value: unknown, maxLength = 1000): string {
  ...
  text = text.replace(/((?:\bbasic|\bdigest|\bnegotiate)\s+)[A-Za-z0-9._~+/=-]{6,}/gi, '$1[redacted]');
  text = text.replace(/([?&](?:username|user|password|pass|passwd|token|api[_-]?key|secret|auth|authorization|session|sessionid|session[_-]?id|cookie|cookies|sid)=)[^&\s]+/gi, '$1[redacted]');
  ...
  text = text.replace(
    /((?:password|passwd|secret|token|api[_-]?key|authorization|auth|session|sessionid|session[_-]?id|cookie|cookies|pin|credentials?)\s*["']?\s*[:=]\s*)(["']?)[^\s,"'&}]+/gi,
    '$1$2[redacted]',
  );
  ...
  // form used in URLs.
  text = text.replace(
    /\[[0-9a-fA-F]{0,4}(?::[0-9a-fA-F]{0,4}){2,}\]|\b(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}\b|\b(?:[0-9a-fA-F]{1,4}:)+:(?:[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4})*)?\b|(?<![0-9a-fA-F:])::(?:[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4})*)?\b/g,
    '[redacted-ip]',
  );
