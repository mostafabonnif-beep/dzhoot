...
} = require('../services/chargily-service');
...
/**
 * Turn a confirmed Chargily payment into exactly one activation code — the same
 * hashed/encrypted-at-rest code mechanism resellers and admins already use.
...
async function fulfillPayment(payment, checkoutData) {
  ...
  payment.status = 'paid';
  payment.paymentMethod = checkoutData?.payment_method || payment.paymentMethod || null;
  ...
  console.log(
    `[payments] Chargily payment fulfilled: payment=${payment._id} plan=${payment.planId} amount=${payment.amount}${payment.currency}`,
  );
}
