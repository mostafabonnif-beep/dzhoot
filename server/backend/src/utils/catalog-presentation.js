...
const REGION_KEYWORDS = [
  ...
  // 2026-09-23 expansion: supplier labels measured live (469 distinct groups,
  // 67% of channels had no country). Turkish alt-spelling, Israeli/Indian
  // language groups, SEA, Nordics-as-one, Africa/Caribbean/LATAM detail.
  [/\bt[uü]rkiye\b/iu, 'TR'],
  [/\b(hebrew|ivrit)\b/iu, 'IL'],
  [/\b(afghanistan|afghan|افغانستان)\b/iu, 'AF'],
  [/\b(bangladesh|bengali|bangla)\b/iu, 'BD'],
  [/\b(tamil|telugu|punjabi|hindi|marathi|gujarati|bhojpuri|kannada|malayalam)\b/iu, 'IN'],
  [/\b(sinhala|ceylon)\b/iu, 'LK'],
  [/\b(thailand|thai\b|siam)\b/iu, 'TH'],
  [/\b(vietnam|vietnamese)\b/iu, 'VN'],
  [/\b(korea|korean|한국)\b/iu, 'KR'],
  [/\b(japan|japanese|nihon)\b/iu, 'JP'],
  [/\b(china|chinese)\b/iu, 'CN'],
  [/\b(australia|aussie)\b/iu, 'AU'],
  [/\b(malaysia|malay\b)\b/iu, 'MY'],
  [/\b(indonesia|indonesian)\b/iu, 'ID'],
  [/\b(philippines|filipino|tagalog)\b/iu, 'PH'],
  [/\b(scandinavia|scandinavian|nordic)\b/iu, 'NORD'],
  [/\b(caribbean|cara[ïi]bes)\b/iu, 'CAR'],
  [/\b(argentina|argentine)\b/iu, 'ARG'],
  [/\b(chile|chilean)\b/iu, 'CL'],
  [/\b(colombia|colombian)\b/iu, 'CO'],
  [/\b(bosnia|bosnian)\b/iu, 'BA'],
  [/\b(ukraine|ukrainian)\b/iu, 'UA'],
  [/\b(armenia|armenian)\b/iu, 'AM'],
  [/\b(azerbaijan|azeri)\b/iu, 'AZ'],
  [/\b(kazakhstan|kazakh)\b/iu, 'KZ'],
  [/\b(south\s*africa|afrikaans)\b/iu, 'ZA'],
  [/\b(nigeria|nigerian)\b/iu, 'NG'],
  [/\b(kenya|kenyan)\b/iu, 'KE'],
  [/\b(ethiopia|ethiopian|amharic)\b/iu, 'ET'],
  [/\b(sudan|sudanese|السودان)\b/iu, 'SD'],
  [/\b(somalia|somali)\b/iu, 'SO'],
  [/\b(albania|albanian|shqip)\b/iu, 'AL'],
];
...
const CATEGORY_RULES = [
  ...
  // 2026-09-23 expansion: multilingual supplier labels measured live — 47% of
  // channels fell into the default category because the rules above only knew
  // English tokens. Turkish/French/Polish/Dutch/Spanish/Arabic VOD and genre
  // labels were the biggest clusters (LUXPLAY, BOX OFFICE, CINE, DIZILER,
  // SPOR, HABER, ENFANTS/KINDEREN/DLA DZIECI, DOCUMENTAIRE...).
  ['أفلام ومسلسلات', /(?:\bcine|kino|box\s*office|\bosn\b|shahid|diziler|\bdizi\b|sezon|season|vod\b|telenovela|novela|luxplay|thriller|\baction\b|pelicula|sinema)/iu],
  ['رياضة', /(?:\bspor\b|eurosport|ligue\s*\d|bundesliga|\bliga\b|campeon|wrestling|\bf\d\b\s*\(?\d?p?\)?)/iu],
  ['أطفال', /(?:kinder|dzieci|crian[çc]as|bambini|\bkika\b|gulli|teletoon|jetx|cbeebies|kartoon|anime|\bmajid\b)/iu],
  ['وثائقي', /(?:documentaire|documentario|documentales|\bdocu\b|docus|museum|nat\s*geo)/iu],
  ['أخبار', /(?:haber|informacyjne|noticias|aktua|telesur)/iu],
  ['موسيقى', /(?:karaoke|musica|musique|m[uü]z[ii]k)/iu],
  ['ترفيه', /(?:pluto|\bgames\b|gaming|esports|\bquiz\b|game\s*show)/iu],
  ['ديني', /(?:christian|catholic|gospel|bibl|sunna|tawhid|peace\s*tv)/iu],
];
