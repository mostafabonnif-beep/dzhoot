...
class Pool:
    ...
    async def check_node(self, node):
        ...
        try:
            ...
            try:
                await socks5_connect(reader, writer,
                                     self.cfg["upstream_check_host"],
                                     self.cfg["upstream_check_port"], timeout)
            finally:
                writer.close()
                try:
            ...
            if not node.healthy and node.consec_ok >= self.cfg["recover_threshold"]:
                node.healthy = True
                self.log(f"node {node.name} RECOVERED ({node.latency_ms} ms)")
            elif not node.healthy:
                self.log(f"node {node.name} check ok {node.consec_ok}/{self.cfg['recover_threshold']} ({node.latency_ms} ms)")
