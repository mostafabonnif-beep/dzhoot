...
**POST** `/admin/app-versions/cache/invalidate` — drop the cached GitHub release lookup
(audit: `APP_VERSION_CACHE_INVALIDATE`).

```json
{ "success": true, "invalidated": ["ghrel:latest"] }
```
...
the operator had to clear the key by hand. Call this endpoint after every publish (or
asset change) so the new release is served immediately; a `500` means the cache was
**not** cleared.
