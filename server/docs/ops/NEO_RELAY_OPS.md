# NEO Relay — تشغيل وتشخيص (موثق 2026-08-19)
...
التطبيق/الخادم → iptables REDIRECT → redsocks (0.0.0.0:12345)
              ...
              → IP منزلك → NEO (cf.business-cloud-neo.ru)
```
...
| `neo-relay.timer` | كل دقيقتين: إذا استمع المنفذ 9000 → يفعّل قواعد iptables (OUTPUT + PREROUTING لعنواني 104.18.22.66/23.66:443 → 12345)، وإلا يزيلها |
| `neo-routes.timer` | إدارة مسارات WireGuard (اختياري) |
| UFW | قاعدة إضافية: `allow from 172.16.0.0/12 to any port 12345 proto tcp` |
| الحاويات | `NODE_OPTIONS=--max-old-space-size=1024 --dns-result-order=ipv4first` — **ipv4first إجباري**: NEO يحل إلى Cloudflare IPv6 (2606:4700::) التي لا تغطيها قواعد التوجيه، وبدون ipv4first يتجاوز البث التوجيه ويُحظر بـ 456 |
...
- حزمة جاهزة: `neo-home-relay-package.zip` (سكربتات Linux/macOS/Windows)
...
- عنوانا Cloudflare (104.18.22.66/23.66) ثابتان حاليًا في DNS — إن تغيّرا، حدّث `NEO_IPS` في `/opt/dzhoot/neo-relay-manage.sh`.
- المصدر "Business Cloud NEO" Active + verified — المزامنة المجدولة كل 6 ساعات.
