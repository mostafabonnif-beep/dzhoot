...
## تنبيهات انتهاء الاشتراك (حلقة التجديد) — إصلاح 2026-09-14

**العَرَض/الخلل:** مهمة `Subscription Expiry Alerts` كانت تُرسل **بريداً** فقط. لكن كل
عميل فعّل كوداً من التطبيق يُنشأ ببريد وهمي (`*@clients.dzhoof.invalid`) — أي أن
التنبيه كان يُرسل إلى عنوان لا وجود له، فلم يصل أحداً.

**الإصلاح:**
1. **إشعار داخل التطبيق أولاً**: لكل مشترك يقترب انتهاؤه يُنشأ `Notification` موجَّه
   (`targetUserId`) + صف `UserNotification` (غير مقروء) → يظهر في صندوق إشعارات التطبيق.
2. **قناة موجّهة جديدة**: `Notification.targetUserId` (null = بثّ للجميع)، و
   `GET /api/v1/me/notifications` يعرض البثّ العام **+** الرسائل الموجّهة لهذا المستخدم فقط.
3. **البريد مكافأة لا أساس**: يُرسل فقط لعنوان قابل للتسليم (تُستثنى نطاقات `.invalid`).
4. **مرة واحدة يومياً**: `Subscription.lastExpiryNoticeOn` يمنع تكرار التنبيه في نفس اليوم.
5. مُلخّص المهمة يعرض القناتين: `sent` (بريد) و`inApp` (داخل التطبيق).

**لماذا يهم:** التجديد أرخص من الاستقطاب. تنبيه يصل قبل 3 أيام يحوّل مشتركاً منتهياً
إلى مشترك متجدد بلا أي تكلفة تسويق — وهذا أهم من أي تحسين سعة.
