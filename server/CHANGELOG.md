...
### Added (an explicit way to invalidate the cached release lookup)
...
- `ghrel:latest` is cached in Redis for 10 minutes and nothing invalidated it, so after
  ...
  hand. `POST /api/v1/admin/app-versions/cache/invalidate` (Admin session, audited as
  `APP_VERSION_CACHE_INVALIDATE`) drops it, and reports `500` rather than pretending the
  cache was cleared. The cache moved to `services/app-release-cache.js` so the admin
  router can invalidate it without importing another router's internals.
