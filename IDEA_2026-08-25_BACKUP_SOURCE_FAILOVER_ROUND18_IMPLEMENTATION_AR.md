...
4. **اختبار حي ليلي**: تعطيل NEO مؤقتًا (mongosh) → فتح قناة مطابَقة → تشتغل من الاحتياطي
   (تحقق host الشريحة) → إرجاع NEO → الجلسة الجديدة ترجع له.

## 5) ملاحظات

- NEO حاليًا في القاعدة: `status: Inactive` + `verificationStatus: blocked` (خطأ TLS من
  الخادم إلى API NEO — يبدو أن الخادم لا يصل لواجهة NEO بينما روابطه المباشرة تعمل للزبائن؛
