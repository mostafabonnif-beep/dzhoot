...
- **الشبكات**: `dzhoof-network` (داخلية) + **`dzhoof-shared-network` خارجية مشتركة مع حاويات مستأجرين آخرين** (api/frontend مرتبطان بها — ملاحظة أمنية معروفة، لم تُعالج بالكامل). **Mongo وRedis محميّان الآن بمصادقة** (`mongod --auth` + `--requirepass`) عبر تجاوز compose الإنتاجي في `/etc/dzhoot/docker-compose.production.yml` — مُفعّل منذ 2026-08-30، وكلمات المرور في `/etc/dzhoot/mongo-app-password` و`mongo-admin-password` و`redis-password` (0600) وليست في Git.
...
- **EPG**: تغطية **30.5% بمعرّف / 23.9% تحلّل لبرامج حيّة** (3,991/16,691 — جولة 7) + **860k برنامج**. المصادر: **37 ملف epgshare01** في `EPG_EXTRA_SOURCES` (beIN/الجزيرة/MENA/أوروبا/أفريقيا/آسيا/أمريكا اللاتينية) — آخر تحديث 74 مصدرًا / فشل 1. السقف البنيوي: OSN/Shahid/STC بلا أدلة عمومية — يحتاج مصدرًا مرخّصًا مدفوعًا.
...
### ميزة تسجيل البث وتحميله (PR #35/#36/#37)
...
- **الحل**: الوكيل يُطبّع قوائم MEDIA:
  ...
  - يُحذف وسم `#EXT-X-ALLOW-CACHE` القديم
  - قوائم MASTER (المتغيرات) تحتفظ بالمسار العودي للتوكنات الفرعية
  - مسار جديد `GET /playback/:token/segments/:seq` يتحقق من التوكن + الجلسة ثم يمرّر المقطع
- **تحقق حي**: مانيفست مطبّع (TARGETDURATION:11) + مقطع 200 (video/mp2t، 7.1MB، رأس TS سليم) + ffmpeg يلعب 25 ثانية بلا توقف (750 إطارًا، النافذة تتقدم).
...
- [~] **EPG**: تُحسّنت 2026-08-30 (جولة 6): 19.9% تحلّل لبرامج حيّة (3,316 قناة) بعد إضافة 6 مصادر epgshare01 وإصلاح `epgIdName` (PR #127). المتبقي بنيوي: OSN/Shahid/STC + قنوات REPLAY بلا أدلة عمومية — يحتاج مصدر EPG عربي مرخّص.
...
- [x] ~~كود تفعيل مُلغى يُقبل في `/pair` و`/verify`~~ — أُغلق 2026-08-30 (PR #129، جولة 7) مع اختبار انحدار
- [x] ~~PIN الاقتران يُسجَّل في السجلات~~ — مُغلق سابقًا (اختبار `tv-pairing-log-safety`)
- [x] ~~`client-redeem`: جلسة 365 يومًا + حد معدل قابل للالتفاف~~ — مُغلق سابقًا (حدّ IP شامل + انتهاء مرتبط بالاشتراك)
- [x] ~~APK بدون R8~~ / ~~Parental PIN بلا salt~~ — مُغلقان سابقًا (R8 + PBKDF2-210k)
- [ ] api/frontend على شبكة Docker مشتركة مع مستأجرين آخرين (عزل ضعيف) — Mongo/Redis أصبحا بمصادقة؛ إزالة `dzhoof-shared-network` تبقى بنيوية
