...
### إصلاح تشغيل التطبيق (PR #40) — القنوات لا تعمل في الـ Android
- **الأعراض**: اللوحة تشغّل القنوات لكن التطبيق يفشل بالرسالة العامة ("البث غير متاح / قد تتوقف مصادر البث"). السجلات أظهرت أن التطبيق يجلب القائمة (200) لكنه **لا يصل لرابط قابل للتشغيل أبدًا** (لا طلبات playback-token).
- **الأسباب (3) وأصلاحها في الخادم** (دون تحديث APK):
  1. القائمة كانت تعيد `channelUrl` فارغًا → الآن عملاء TV يحصلون على **روابط مُرمّزة قابلة للتشغيل** (توكن لكل قناة + بدائل؛ التوكنات لكل مستخدم وتنتهي؛ ذاكرة التخزين المؤقت تبقى للويب فقط).
  2. التوكنات المسبقة بلا جلسة Redis → مسار `/tv/playback` الآن **ينشئ الجلسة كسولًا** عند أول تشغيل (ضمن حد التزامن) بدل 429.
  3. إصدارات تطبيق أقدم ترسل Mongo _id → مسار `playback-token` يحلّ الآن **بالـ _id أو channelId**.
- **تحقق حي**: القائمة تعيد روابط مُرمّزة؛ تشغيلها بلا headers (مثل ExoPlayer) → مانيفست حي + مقاطع فيديو؛ ومسارا _id وxt يعملان (200).
- ملاحظة: لقطة AL KASS "SPO: Al Kass 1 HD" تُشغَّل في اللوحة (تم سابقًا) — التطبيق على إصدار 1.0.8 (يحتوي HLS) أو أحدث يجب أن يعمل الآن.
