...
## ⚡ تحديث — الجولة 10 (نفس اليوم): تحسين لوحتي الأدمن والموزع

- **الأدمن**: جدول "مبيعات المحلات" في لوحة القيادة (تفعيلات الشهر/الإجمالي + قيمة المشتريات لكل محل) + عمود "قيمة المشتريات" في قائمة المحلات.
- **الموزع**: زر نسخ كود فردي + بحث في الدفعات (بالرقم أو الباقة) + بطاقة "تفعيلات الشهر".
- النشر `v1.0.1-20260824T170835Z` | GitHub `48e285e` + تقرير الجولة 10 | اختبارات 251/251 | تحقق حي كامل (محل → منح → توليد R10X → تفعيل → مبيعات 1 + مشتريات 4000 ✓ → تنظيف).

---

## ⚡ تحديث — الجولة 11 (نفس اليوم): التنبيهات + البريد + الصلاحية + الأسعار

- **سبب تعطل التنبيهات كان**: `ALERT_WEBHOOK_URL` فارغ + بيانات Brevo SMTP ناقصة في `.env` — بلا أي طريقة لضبطها من الواجهة.
- **أُضيف**: قسم "التنبيهات والإشعارات" في صفحة الإعدادات (webhook + Brevo user/pass + MAIL_FROM) يُحفظ في القاعدة ويُقرأ عند التشغيل (alert-notifier + email) + زرا "تنبيه تجريبي" و"بريد تجريبي". حقل "مدة صلاحية أكواد الموزعين" (code_expiry_days 1-365). `/health?details=true` يعكس webhook المحفوظ الآن.
- **الأسعار مضبوطة (مقترحة)**: تجربة 0 / شهري 500 / 6 أشهر 2,500 / سنوي 4,500 دج — `pricesSet: true`، قابلة للتعديل من صفحة الباقات.
- النشر `v1.0.1-20260824T180501Z` | GitHub `f6747a4` + `7d5b038` + تقرير الجولة 11 | اختبارات 251/251 | تحقق حي: حفظ webhook → health=true → تنبيه تجريبي برسالة واضحة → مسح → health=false ✓.
- **ما يحتاجه المستخدم للتفعيل النهائي**: رابط webhook حقيقي (Discord/Slack) + مفاتيح Brevo — من صفحة الإعدادات مباشرة.
